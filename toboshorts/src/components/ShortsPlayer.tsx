...
export default function ShortsPlayer({ video, isActive, onEnded }: ShortsPlayerProps) {
  ...
  useEffect(() => {
    ...
    if (isActive) {
      // Small delay prevents multiple videos from playing sound during fast scrolling
      ...
      if (videoRef.current) {
        ...
        // Resetting currentTime ensures we don't hear a "blip" of old audio when returning
